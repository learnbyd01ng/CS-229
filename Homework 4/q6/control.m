...
NO_LEARNING_THRESHOLD = 30; % Increased to 30 to give longer learning curve
...
P1 = ones(NUM_STATES,NUM_STATES)/NUM_STATES;    % P1 is P under action 1
P2 = P1;                                        % P2 is P under action 2
%%%% END YOUR CODE %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% here, and say the overall algorithm has converged. 
transition_counts1 = zeros(NUM_STATES,NUM_STATES);
transition_counts2 = zeros(NUM_STATES,NUM_STATES);
num_learned = 0;

while num_learned < NO_LEARNING_THRESHOLD
% while (number of consecutive no learning trials < NO_LEARNING_THRESHOLD)


  %%% CODE HERE: Write code to choose action (1 or 2) %%%
  % Choose the action for which the expected Value function of the output
  % state is highest.
  v1 = P1(state,:)*V;
  v2 = P2(state,:)*V;
  
  if (v1 == v2)
    action = round(rand(1,1))+1;
  elseif v1 > v2
    action=1;
  else
    action=2;
  ...
  if action == 1,
    transition_counts1(state,new_state) = ...
                transition_counts1(state,new_state)+1;
  else
    transition_counts2(state,new_state) = ...
                transition_counts2(state,new_state)+1;
  end
  ...
  if (new_state==NUM_STATES)
    ...
    Q1 = sum(transition_counts1,2);
    Q2 = sum(transition_counts2,2);
    for i = 1:NUM_STATES
        P1(i,:) = transition_counts1(i,:)/Q1(i);
        P2(i,:) = transition_counts2(i,:)/Q2(i);
    end
    P1(isnan(P1)) = 1/NUM_STATES;
    P2(isnan(P2)) = 1/NUM_STATES;
    ...
    newV = GAMMA*max(P1*V, P2*V);
    newV(end) = -1;
    i = 0;
    
    while max(abs(newV(1:end-1) - V(1:end-1))) > TOLERANCE
        V = newV;
        newV = GAMMA*max(P1*V, P2*V);
        newV(end) = -1;
        i = i + 1;
